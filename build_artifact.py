...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.1-lua-gumbo-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.1-lua-gumbo-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.1-lua-gumbo-*.xml" in "%s".' % strCwd)
...
for strMatch in glob.iglob(os.path.join(strCwd, 'lua5.4-lua-gumbo-*.xml')):
    os.remove(strMatch)
...
astrMatch = glob.glob(os.path.join(strCwd, 'lua5.4-lua-gumbo-*.xml'))
if len(astrMatch) != 1:
    raise Exception('No match found for "lua5.4-lua-gumbo-*.xml" in "%s".' % strCwd)
